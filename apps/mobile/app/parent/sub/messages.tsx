...
import { P_MESSAGES } from "@/lib/parent-data";
  ...
  return (
    <View style={{ flex: 1, backgroundColor: ukColors.bg }}>
      <View style={st.head}>
        ...
        <View style={st.avatar}><Text style={{ color: "#fff", fontWeight: "800", fontSize: 14 }}>{child.coachInitials}</Text></View>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 15, fontWeight: "800", color: ukColors.text }}>{child.coach}</Text>
          <Text style={{ fontSize: 11.5, fontWeight: "700", color: ukColors.muted }}>{child.name.split(" ")[0]} — Koçu</Text>
        </View>
      </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        <View style={st.inputBar}>
          <TextInput style={st.input} placeholder="Koça mesaj yaz…" placeholderTextColor={ukColors.faint} value={val} onChangeText={setVal} onSubmitEditing={send} returnKeyType="send" />
