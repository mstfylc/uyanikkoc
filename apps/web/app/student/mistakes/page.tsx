...
export default function StudentMistakesPage() {
  ...
  return (
    <div className="stack rise">
      ...
      <div className="yd-summary">
        {[
          ...
          <div key={label as string} className="yd-sum">
            ...
            <div>
              <div className="yd-sum-value tnum">{isLoading ? "-" : value}</div>
              <div className="yd-sum-label">{label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <UkSection title="Hata Frekansı" sub="Son 14 günde hangi tür hatayı daha çok yapıyorsun" action={<Badge tone="primary">{frequency.total} yanlış</Badge>}>
